...
int main(int argc, char **argv) {
    ...
    int i, j, t, k, iter = 200, flag = 0, d = 0, Pnum = 0;
    double n;
    char c;
    float tempk, tempiter;
    double **arr, **oldArr, **newArr;
...
int isAllDigits(char* str) {
    int i;
    if (strlen(str) == 2 && str[0] == '.') return 0;
    for (i = 0; str[i] != '\0'; i++) {
        if (str[i] < '0' || str[i] > '9') {
            if (str[i] == '.') continue;
            else return 0;
